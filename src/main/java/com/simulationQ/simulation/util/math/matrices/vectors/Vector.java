 ...
 */
package com.simulationQ.simulation.util.math.matrices.vectors;
                    ...
                    Iterable< ComplexNumber >
{
    ...
    public Vector tensorProduct ( Vector a )
    {
        ...
        for ( int i = 0 ; i < scaledVectors.length ; i++ )
        {
            scaledVectors[i] =
                new Vector(
                    a.multiplyWithScalar( scalars[i] )
                        .getMatrix()[0] );
        }
    ...
    //
    // Vector b = new Vector( a.multiplyWithScalar(
    // ComplexNumber.REAL_UNIT.add(
